  ...
  for (int i = 0; i < 2; i++) {
    ...
    while (true) {
      ...
      if ((fmask & tag) == (fmask & ftag)) {
        struct lfs_diskoff temp = (struct lfs_diskoff){.block = dir->pair[0], .off = off + sizeof(tag)};
        int res = cb(data, tag, &temp);
        if (res < 0) {
...
static int lfs_dir_find_match(void *data, lfs_tag_t tag, const void *buffer) {
  ...
  lfs_t *lfs = name->lfs;
  const struct lfs_diskoff *disk = (const struct lfs_diskoff *)buffer;
...
static lfs_stag_t lfs_dir_find(lfs_t *lfs, lfs_mdir_t *dir, const char **path, uint16_t *id) {
  ...
  nextname:
    ...
    *path = name;
    ...
    while (true) {
      struct lfs_dir_find_match temp = (struct lfs_dir_find_match){lfs, name, namelen};
      tag = lfs_dir_fetchmatch(lfs, dir, dir->tail, LFS_MKTAG(0x780, 0, 0), LFS_MKTAG(LFS_TYPE_NAME, 0, namelen),
                               // are we last name?
                               (strchr(name, '/') == NULL) ? id : NULL, lfs_dir_find_match, &temp);
      if (tag < 0) {
        return tag;
      }
  ...
  while (true) {
    {
      ...
      struct lfs_dir_commit_commit temp = (struct lfs_dir_commit_commit){lfs, &commit};
      err = lfs_dir_traverse(lfs, source, 0, 0xffffffff, attrs, attrcount, LFS_MKTAG(0x400, 0x3ff, 0),
                             LFS_MKTAG(LFS_TYPE_NAME, 0, 0), begin, end, -begin, lfs_dir_commit_commit, &temp);
      if (err) {
        if (err == LFS_ERR_CORRUPT) {
          goto relocate;
        }
...
static int lfs_dir_commit(lfs_t *lfs, lfs_mdir_t *dir, const struct lfs_mattr *attrs, int attrcount) {
  ...
  if (dir->erased || dir->count >= 0xff) {
    ...
    struct lfs_dir_commit_commit temp = (struct lfs_dir_commit_commit){lfs, &commit};
    int err =
        lfs_dir_traverse(lfs, dir, dir->off, dir->etag, attrs, attrcount, 0, 0, 0, 0, 0, lfs_dir_commit_commit, &temp);
    lfs_pair_fromle32(dir->tail);
    if (err) {
...
static int lfs_ctz_find(lfs_t *lfs, const lfs_cache_t *pcache, lfs_cache_t *rcache, lfs_block_t head, lfs_size_t size,
  ...
  lfs_off_t temp = (lfs_off_t){size - 1};
  lfs_off_t current = lfs_ctz_index(lfs, &temp);
...
static int lfs_ctz_traverse(lfs_t *lfs, const lfs_cache_t *pcache, lfs_cache_t *rcache, lfs_block_t head,
  ...
  lfs_off_t temp = (lfs_off_t){size - 1};
  lfs_off_t index = lfs_ctz_index(lfs, &temp);
...
lfs_ssize_t lfs_file_write(lfs_t *lfs, lfs_file_t *file, const void *buffer, lfs_size_t size) {
  ...
  if (!(file->flags & LFS_F_WRITING) && file->pos > file->ctz.size) {
    ...
    while (file->pos < pos) {
      ...
      lfs_ssize_t res = lfs_file_write(lfs, file, &temp, 1);
      if (res < 0) {
  ...
  if ((file->flags & LFS_F_INLINE) && lfs_max(file->pos + nsize, file->ctz.size) >
...
int lfs_format(lfs_t *lfs, const struct lfs_config *cfg) {
  LFS_TRACE("lfs_format(%p, %p {.context=%p, "
            ...
            (void *)(uintptr_t)cfg->erase, (void *)(uintptr_t)cfg->sync, cfg->read_size, cfg->prog_size,
            cfg->block_size, cfg->block_count, cfg->block_cycles, cfg->cache_size, cfg->lookahead_size,
            cfg->read_buffer, cfg->prog_buffer, cfg->lookahead_buffer, cfg->name_max, cfg->file_max, cfg->attr_max);
  int err = 0;
  {
...
int lfs_mount(lfs_t *lfs, const struct lfs_config *cfg) {
  ...
  while (!lfs_pair_isnull(dir.tail)) {
    ...
    struct lfs_dir_find_match temp = (struct lfs_dir_find_match){lfs, "littlefs", 8};
    lfs_stag_t tag = lfs_dir_fetchmatch(lfs, &dir, dir.tail, LFS_MKTAG(0x7ff, 0x3ff, 0),
                                        LFS_MKTAG(LFS_TYPE_SUPERBLOCK, 0, 8), NULL, lfs_dir_find_match, &temp);
    if (tag < 0) {
      err = tag;
      goto cleanup;
    }
...
static lfs_stag_t lfs_fs_parent(lfs_t *lfs, const lfs_block_t pair[2], lfs_mdir_t *parent) {
  ...
  while (!lfs_pair_isnull(parent->tail)) {
    ...
    struct lfs_fs_parent_match temp = (struct lfs_fs_parent_match){lfs, {pair[0], pair[1]}};
    lfs_stag_t tag = lfs_dir_fetchmatch(lfs, parent, parent->tail, LFS_MKTAG(0x7ff, 0, 0x3ff),
                                        LFS_MKTAG(LFS_TYPE_DIRSTRUCT, 0, 8), NULL, lfs_fs_parent_match, &temp);
