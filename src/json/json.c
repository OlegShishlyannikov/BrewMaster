...
*/
static int jfes_check_configuration(const jfes_config_t *config) { return config && config->jfes_malloc && config->jfes_free; }
...
static int jfes_is_boolean(const char *data, jfes_size_t length) {
  ...
  return length == jfes_strlen(JFES_TRUE_VALUE) && jfes_memcmp(data, JFES_TRUE_VALUE, length) == 0 || length == jfes_strlen(JFES_FALSE_VALUE) && jfes_memcmp(data, JFES_FALSE_VALUE, length) == 0;
}
...
static int jfes_is_integer(const char *data, jfes_size_t length) {
  ...
  if (length > offset + 2 && data[offset] == '0' && (data[offset + 1] == 'x' || data[offset + 1] == 'X')) {
    hexadecimal_case = 1;
  }
  ...
  for (jfes_size_t i = offset; i < length; i++) {
    if ((data[i] >= '0' && data[i] <= '9') || (hexadecimal_case && ((data[i] >= 'A' && data[i] <= 'F') || (data[i] >= 'a' && data[i] <= 'f'))) ||
  ...
  for (jfes_size_t i = offset; i < length; i++) {
    if (data[i] < '0' || data[i] > '9') {
      if (data[i] == '.' && !dot_already_been) {
        dot_already_been = 1;
        continue;
      } else if ((data[i] == 'e' || data[i] == 'E') && i + 2 < length && (data[i + 1] == '+' || data[i + 1] == '-') && !exp_already_been) {
        exp_already_been = 1;
        i++;
        continue;
      }
...
static int jfes_string_to_integer(const char *data, jfes_size_t length) {
  ...
  for (jfes_size_t i = offset; i < length; i++) {
    char c = data[i];
    if ((c >= '0' && c <= '9') || (base == jfes_hexadecimal_integer && ((c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F')))) {
      int value = 0;
      if (c >= '0' && c <= '9') {
        value = c - '0';
      } else if (c >= 'A' && c <= 'F') {
...
*/
static const char *jfes_boolean_to_string(int value) { return value ? JFES_TRUE_VALUE : JFES_FALSE_VALUE; }
...
*/
static char *jfes_integer_to_string_r(int value, char *output, jfes_size_t output_size, jfes_size_t nzeroes) {
...
static char *jfes_integer_to_string(int value) {
  ...
  return jfes_integer_to_string_r(value, &buf[0], JFES_MAX_DIGITS, 0u);
}
...
static char *jfes_double_to_string_r(double value, char *output, jfes_size_t output_size, double precision_eps) {
  ...
  char *int_value_s = jfes_integer_to_string_r(int_value, &output[0], output_size, 0u);
  if (!int_value_s) {
    return JFES_NULL;
  }
  ...
  char *fractional_value_s = jfes_integer_to_string_r(fractional_int, &output[value_length + 1], output_size - value_length - 1, nzeroes);
...
/**
    ...
    \return         jfes_success if everything is OK.
*/
static jfes_status_t jfes_parse_primitive(jfes_parser_t *parser, const char *json, jfes_size_t length, jfes_token_t *tokens, jfes_size_t max_tokens_count) {
...
/**
    ...
    \return         jfes_success if everything is OK.
*/
static jfes_status_t jfes_parse_string(jfes_parser_t *parser, const char *json, jfes_size_t length, jfes_token_t *tokens, jfes_size_t max_tokens_count) {
  if (!parser || !json || length == 0 || !tokens || max_tokens_count == 0) {
    return jfes_invalid_arguments;
  }
    ...
    if (c == '\"') {
      ...
      case 'u':
        ...
        for (jfes_size_t i = 0; i < 4 && parser->pos < length && json[parser->pos] != '\0'; i++, parser->pos++) {
          ...
          if ((symbol < (int)'0' || symbol > (int)'9') && (symbol < (int)'A' || symbol > (int)'F') && (symbol < (int)'a' || symbol > (int)'f')) {
            parser->pos = start;
...
int jfes_status_is_good(jfes_status_t status) { return status == jfes_success; }

int jfes_status_is_bad(jfes_status_t status) { return !jfes_status_is_good(status); }
...
jfes_status_t jfes_parse_tokens(jfes_parser_t *parser, const char *json, jfes_size_t length, jfes_token_t *tokens, jfes_size_t *max_tokens_count) {
  ...
  while (parser->pos < length && json[parser->pos] != '\0') {
    ...
    case ',': {
      if (parser->superior_token != -1 && tokens[parser->superior_token].type != jfes_type_array && tokens[parser->superior_token].type != jfes_type_object) {
...
static jfes_status_t jfes_create_node(jfes_tokens_data_t *tokens_data, jfes_value_t *value) {
  ...
  case jfes_type_boolean:
    value->data.bool_val = jfes_string_to_boolean(tokens_data->json_data + token->start, token->end - token->start);
    break;

  case jfes_type_integer:
    value->data.int_val = jfes_string_to_integer(tokens_data->json_data + token->start, token->end - token->start);
    break;

  case jfes_type_double:
    value->data.double_val = jfes_string_to_double(tokens_data->json_data + token->start, token->end - token->start);
    break;

  case jfes_type_string:
    jfes_create_string(tokens_data->config, &value->data.string_val, tokens_data->json_data + token->start, token->end - token->start);
    break;
  ...
  case jfes_type_object:
    value->parent = JFES_NULL;
    value->name = JFES_NULL;
    ...
    if (token->size > 0) {
      ...
      for (jfes_size_t i = 0; i < token->size; i++) {
        ...
        jfes_create_string(tokens_data->config, &item->key, tokens_data->json_data + key_token->start, key_length);
        ...
        value->data.object_val->items[i]->value->parent = value;
        value->data.object_val->items[i]->value->name = value->data.object_val->items[i]->key.data;
      }
    }
    break;
...
jfes_status_t jfes_parse_to_value(const jfes_config_t *config, const char *json, jfes_size_t length, jfes_value_t *value) {
...
jfes_value_t *jfes_create_null_value(const jfes_config_t *config) {
  ...
  result->parent = JFES_NULL;
  result->name = JFES_NULL;
...
jfes_value_t *jfes_get_parent(const jfes_value_t *value) { return value->parent; }
const char *jfes_get_name(const jfes_value_t *value) { return value->name; }
...
jfes_object_map_t *jfes_get_mapped_child(const jfes_value_t *value, const char *key, jfes_size_t key_length) {
  ...
  for (jfes_size_t i = 0; i < value->data.object_val->count; i++) {
    ...
    if (item && (item->key.size - 1) == key_length && jfes_memcmp(item->key.data, key, key_length) == 0) {
      return item;
    }
  }
...
jfes_status_t jfes_set_object_property(const jfes_config_t *config, jfes_value_t *value, jfes_value_t *item, const char *key, jfes_size_t key_length) {
  ...
  item->parent = value;
  item->name = object_map->key.data;

  return jfes_success;
}

jfes_status_t jfes_remove_object_property(const jfes_config_t *config, const jfes_value_t *value, const char *key, jfes_size_t key_length) {
...
\return         jfes_success if everything is OK.
*/
static jfes_status_t jfes_value_to_stream_helper(const jfes_value_t *value, jfes_stringstream_t *stream, int beautiful, jfes_size_t indent, const char *indent_string);
...
/**
    ...
    \return         jfes_success if everything is OK.
*/
static jfes_status_t jfes_array_value_to_stream_helper(const jfes_value_t *value, jfes_stringstream_t *stream, int beautiful, jfes_size_t indent, const char *indent_string) {
  ...
  if (beautiful) {
    if (value->data.array_val->count > 0 && (value->data.array_val->items[0]->type == jfes_type_object || value->data.array_val->items[0]->type == jfes_type_array)) {
  ...
  for (jfes_size_t i = 0; i < value->data.array_val->count; i++) {
    ...
    if (beautiful) {
      if ((i < value->data.array_val->count - 1 && (value->data.array_val->items[i + 1]->type == jfes_type_array || value->data.array_val->items[i + 1]->type == jfes_type_object)) ||
          (i == value->data.array_val->count - 1 && with_indent) || (value->data.array_val->items[i]->type == jfes_type_array || value->data.array_val->items[i]->type == jfes_type_object)) {
        jfes_add_to_stringstream(stream, "\n", 0);
        with_indent = 1;
      } else {
        jfes_add_to_stringstream(stream, " ", 0);
...
/**
    ...
    \return         jfes_success if everything is OK.
*/
static jfes_status_t jfes_object_value_to_stream_helper(const jfes_value_t *value, jfes_stringstream_t *stream, int beautiful, jfes_size_t indent, const char *indent_string) {
...
static jfes_status_t jfes_value_to_stream_helper(const jfes_value_t *value, jfes_stringstream_t *stream, int beautiful, jfes_size_t indent, const char *indent_string) {
  ...
  case jfes_type_integer: {
    ...
    return jfes_add_to_stringstream(stream, jfes_integer_to_string_r(value->data.int_val, &buffer[0], JFES_MAX_DIGITS, 0u), 0);
  }

  case jfes_type_double: {
    char buffer[JFES_MAX_DIGITS];
    return jfes_add_to_stringstream(stream, jfes_double_to_string_r(value->data.double_val, &buffer[0], JFES_MAX_DIGITS, JFES_DOUBLE_PRECISION), 0);
  }
...
/**
    ...
    \return         jfes_success if everything is OK.
*/
static jfes_status_t jfes_value_to_stream(const jfes_value_t *value, jfes_stringstream_t *stream, int beautiful) { return jfes_value_to_stream_helper(value, stream, beautiful, 0, "    "); }
